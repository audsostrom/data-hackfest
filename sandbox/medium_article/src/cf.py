...
def train():
    ...
    for epoch in range(config.epochs):
        ...
        wandb.log({"epoch": epoch, "validation_loss": avg_val_loss})
...
def sweep():
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "embedding_size": {"value": 128}
        }
    }
    sweep_id = wandb.sweep(sweep_config, project=f"sweep_{timestamp}")
...
def run_single_experiment():
    ...
    run_name = f"single_run-lr_{config['learning_rate']}-bs_{config['batch_size']}-emb_{config['embedding_size']}-ep_{config['epochs']}"
    wandb.run.name = run_name
